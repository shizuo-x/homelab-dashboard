...
const SshLauncherWidget = ({ widget }) => {
  ...
  const handleConnect = (host) => {
    ...
    eventBus.dispatch('run-command', command);
  };

  return (
    <div className="p-4 flex flex-col h-full">
      <div className="flex justify-between items-center mb-3">
        <h2 className="font-bold text-green-400 flex items-center"><FaRocket className="mr-2" /> SSH LAUNCHER</h2>
      </div>
      <div className="flex-grow overflow-y-auto pr-2 custom-scroll space-y-2">
        {hosts.length > 0 ? (
          hosts.map(host => (
            <button 
              key={host.name}
              onClick={() => handleConnect(host)} // <-- Add the onClick handler
            ...
            >
              <span><i className="fas fa-server mr-2"></i> {host.name.toUpperCase()}</span>
              <FaChevronRight />
            </button>
          ))
        ) : (
