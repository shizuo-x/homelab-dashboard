...
// ServiceStatusForm is unchanged...
const ServiceStatusForm = ({ initialConfig, onSave, onCancel }) => { const [services, setServices] = useState(initialConfig?.services || []); const [newName, setNewName] = useState(''); const [newUrl, setNewUrl] = useState(''); const handleAddService = () => { if (newName && newUrl) { setServices(current => [...current, { name: newName, url: newUrl }]); setNewName(''); setNewUrl(''); } }; const handleRemoveService = (serviceNameToRemove) => { setServices(current => current.filter(s => s.name !== serviceNameToRemove)); }; const handleSave = () => { onSave({ services }); }; return ( <div> <div className="mb-4 max-h-48 overflow-y-auto custom-scroll pr-2 border-b border-slate-700 pb-2"> {services.map(service => ( <div key={service.name} className="flex items-center justify-between bg-slate-900 p-2 rounded mb-2"> <div><p className="font-bold">{service.name}</p><p className="text-xs text-gray-500">{service.url}</p></div> <button onClick={() => handleRemoveService(service.name)} className="p-2 text-red-500 hover:bg-red-900 rounded-full"><FaTrash /></button> </div> ))} </div> <div className="grid grid-cols-2 gap-4 mb-4"> <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="New Service Name" className="w-full bg-slate-900 rounded p-2 border border-slate-700" /> <input type="text" value={newUrl} onChange={e => setNewUrl(e.target.value)} placeholder="https://service.url" className="w-full bg-slate-900 rounded p-2 border border-slate-700" /> </div> <button type="button" onClick={handleAddService} className="w-full mb-4 py-2 bg-blue-800 hover:bg-blue-700 rounded">Add Service to List</button> <div className="flex justify-end gap-4"><button type="button" onClick={onCancel} className="px-4 py-2 bg-gray-600 rounded">Cancel</button><button type="button" onClick={handleSave} className="px-4 py-2 bg-green-600 rounded">Save Changes</button></div> </div> ); };

// --- NEW FORM FOR SSH LAUNCHER ---
const SshLauncherForm = ({ initialConfig, onSave, onCancel }) => {
  const [hosts, setHosts] = useState(initialConfig?.hosts || []);
  ...
  const handleAddHost = () => {
    if (name && user && host) {
      setHosts(current => [...current, { name, user, host }]);
      setName(''); setUser(''); setHost('');
    }
  };

  const handleRemoveHost = (hostNameToRemove) => {
    setHosts(current => current.filter(h => h.name !== hostNameToRemove));
  };
  ...
  return (
    <div>
      <div className="mb-4 max-h-48 overflow-y-auto custom-scroll pr-2 border-b border-slate-700 pb-2">
        {hosts.map(h => (
          <div key={h.name} className="flex items-center justify-between bg-slate-900 p-2 rounded mb-2">
            <div><p className="font-bold">{h.name}</p><p className="text-xs text-gray-500">{h.user}@{h.host}</p></div>
            <button onClick={() => handleRemoveHost(h.name)} className="p-2 text-red-500 hover:bg-red-900 rounded-full"><FaTrash /></button>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-3 gap-4 mb-4">
        <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Display Name" className="w-full bg-slate-900 rounded p-2 border border-slate-700" />
        <input type="text" value={user} onChange={e => setUser(e.target.value)} placeholder="user" className="w-full bg-slate-900 rounded p-2 border border-slate-700" />
        <input type="text" value={host} onChange={e => setHost(e.target.value)} placeholder="hostname or IP" className="w-full bg-slate-900 rounded p-2 border border-slate-700" />
      </div>
      <button type="button" onClick={handleAddHost} className="w-full mb-4 py-2 bg-blue-800 hover:bg-blue-700 rounded">Add Host to List</button>
